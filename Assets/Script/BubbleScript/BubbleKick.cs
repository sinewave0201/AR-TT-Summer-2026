...
public class BubbleKick : MonoBehaviour
{
    [Header("Kick")]
    ...
    {
        if (!interactionEnabled || dragging || rb.isKinematic)
        {
            return;
        }
    ...
    public void ResetBubblePosition()
    {
        ...
        BubbleBurn bubbleBurn = GetComponent<BubbleBurn>();
        if (bubbleBurn != null)
        {
            bubbleBurn.ResetBurnState();
        }
    }
