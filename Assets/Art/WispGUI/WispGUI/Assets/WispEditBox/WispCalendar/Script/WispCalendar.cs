...
public class WispCalendar : WispVisualComponent 
{
	...
	[Header("Bubble Highlights")]
	[SerializeField] private bool highlightBubbleDates = true;
	[SerializeField] private VaultManager vaultManager;
	[SerializeField] private Color bubbleDateColor = new Color(1f, 0.94f, 0.55f, 1f);
	[SerializeField] private Color bubbleDateTextColor = new Color(0.18f, 0.14f, 0.04f, 1f);

	[Header("Render Pipeline Compatibility")]
	[SerializeField] private bool disableStyleMaterials = true;
    ...
    {
        ApplyStyle();
		StartCoroutine(ClearStyleMaterialsNextFrame());
	}
	...
	protected void PaintButtons (bool ParamUpdateInitialTransitionButtons = true)
	{
		...
		{	
			...
			if (b.ActiveMonth)
			{
				...
				DateTime buttonDate = new DateTime(currentMonth.Year, currentMonth.Month, Int32.Parse(b.Text.text));
				if (HasBubbleOnDate(buttonDate))
				{
					b.GetComponent<Image>().color = bubbleDateColor;
					b.Text.color = bubbleDateTextColor;

					arrivingTransitionButtons[key].GetComponent<Image>().color = bubbleDateColor;
					arrivingTransitionButtons[key].GetComponentInChildren<TMPro.TextMeshProUGUI>().color = bubbleDateTextColor;
				}
			}
			else 
			{
		...
		if (currentlySelectedDayButton != null && currentMonth.Month == selectedDate.Month) {
			...
			if (HasBubbleOnDate(selectedDate))
			{
				currentlySelectedDayButton.GetComponent<Image>().color = bubbleDateColor;
				currentlySelectedDayButton.Text.color = bubbleDateTextColor;
			}
		}
	...
	public void RefreshBubbleDateHighlights()
	{
		if (isInitialized)
		{
			PaintButtons();
	...
	private bool HasBubbleOnDate(DateTime date)
	{
		if (!highlightBubbleDates || vaultManager == null)
		{
			return false;
		}
	...
	private void ClearStyleMaterials()
	{
		if (!disableStyleMaterials)
		{
			return;
		}
	...
	private IEnumerator ClearStyleMaterialsNextFrame()
	{
		yield return null;
		ClearStyleMaterials();
